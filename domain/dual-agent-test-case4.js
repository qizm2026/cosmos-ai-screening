#!/usr/bin/env node
/**
 * COSMO 双Agent自动化测试 — 案例四：苏雨桐（重度范围，期望26分）
 *
...
async function main() {
  ...
  console.log('COSMO 双Agent测试 — 案例四：苏雨桐（重度范围，期望26分）')
...
- 你不会主动提及自伤细节——但如果被问到Q9相关内容，你会如实说出`
  ...
  userMessages.push({
    role: 'user',
    content: `心理老师对你说："${r.reply}"\n\n请以苏雨桐的身份自然回复。你极度内向、被动，语气低沉。简单回应一下就好——不需要多说。`
  })
  ...
  while (turn < MAX_TURNS) {
    ...
    userMessages.push({
      role: 'user',
      content: `心理老师说："${cosmoSaid.slice(0, 600)}"\n\n请以苏雨桐的身份自然回复。记住：你是15岁初三女生，极度内向、自我封闭。你回答简短是因为没有力气说更多，不是抗拒。老师追问了就如实给出信息——语气平淡低沉，像被掏空了一样。你不会渲染惨状，但说出的事实会让人感到深深的绝望。`
    })
...
// 苏雨桐期望得分映射
const EXPECTED = { Q1: 3, Q2: 3, Q3: 3, Q4: 3, Q5: 3, Q6: 3, Q7: 2, Q8: 3, Q9: 3, total: 26 }
...
function printReport(chatHistory, sysLog, score, report) {
  ...
  console.log(`测试对象: 案例四 — 苏雨桐（重度范围，期望26分）`)
